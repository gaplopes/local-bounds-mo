/**
 * @file bound_set.hpp
 * @brief Core class for maintaining local bounds in multiobjective
 * optimization.
 *
...
public:
   ...
   * space). For MAXIMIZE: the ideal point (lower bound of search space).
   */
  /**
   ...
   * @param reference_point For MINIMIZE: the nadir point M.
   *                        For MAXIMIZE: the ideal point m.
   * @param anti_reference  For MINIMIZE: the ideal point m (lower bound of
   * search space). For MAXIMIZE: the nadir point M (upper bound of search
   * space).
   */
   ...
   *
   * Implements Algorithm 4 (Redundancy Avoidance - General Position) from the
   * paper. This algorithm avoids the filtering step by leveraging the structure
   * of bounds under the general position assumption (SA). It maintains
   ...
   *
   * Implements Algorithm 5 (Redundancy Avoidance - General Case) from the
   * paper. This algorithm theoretically avoids the filtering step entirely by
   * tracking the sets of points Z^j(u) that define each component of each local
   * bound.
   *
   * @param point The new nondominated point.
   */
   ...
   *
   * Selection rule:
   *   - p <= 5: Algorithm 2 (RE) — better cache locality and linear bucketing
   * logic.
   ...
   *
   * For MINIMIZE: returns true if the point is strictly dominated by some
   * bound. For MAXIMIZE: returns true if the point strictly dominates some
   * bound.
   *
...
private:
  ...
  void update_redundancy_elimination(std::vector<LocalBound<T>> &current_bounds,
    ...
    for (std::size_t j = 0; j < dimensions_; ++j) {
      B_idx[j].reserve(current_bounds.size());
    }
    ...
    for (std::size_t i = 0; i < current_bounds.size(); ++i) {
      ...
      } else if (weakly_dominates<T, Sense>(point.coordinates,
                                            current_bounds[i].coordinates)) {
        for (std::size_t j = 0; j < dimensions_; ++j) {
          if (point.coordinates[j] == current_bounds[i].coordinates[j]) {
            B_idx[j].push_back(
                i); // u in B mapped to specific equality dimension j
          }
        }
      }
    }
    ...
    P.reserve(A_idx.size() * dimensions_ / 2); // heuristic reserve
    ...
    for (std::size_t idx : A_idx) {
      ...
      for (std::size_t j = 0; j < dimensions_; ++j) {
        ...
        // Filter against B_j (weakly dominated bounds with equality at
        // dimension j)
        if (!dominates) {
    ...
    // Step 3: Remove the original dominated bounds (Set A) from current_bounds
    // Because A_idx was populated in ascending order, reverse iteration is
    // perfectly safe here.
    ...
    // Step 4: Append the filtered new bounds (Set P) into current_bounds
  ...
  void update_redundancy_elimination_enhanced(
      ...
      for (std::size_t j = 0; j < dimensions_; ++j) {
        if (point.coordinates[j] == u.coordinates[j]) {
          ...
          for (std::size_t k = 0; k < dimensions_; ++k) {
            if (k == j)
              continue;
            if (!detail::is_better<T, Sense>(point.coordinates[k],
                                             u.coordinates[k])) {
    ...
    // Step 3: Lazy Generation and Filtering
  ...
  update_redundancy_avoidance_sa(std::vector<LocalBound<T>> &current_bounds,
    ...
    A_idx.reserve(current_bounds.size());
    for (std::size_t i = 0; i < current_bounds.size(); ++i) {
      if (strictly_dominates<T, Sense>(z_bar.coordinates,
                                       current_bounds[i].coordinates)) {
        A_idx.push_back(i);
      }
    }
    ...
    for (std::size_t idx : A_idx) {
      ...
      for (std::size_t j = 0; j < dimensions_; ++j) {
        ...
        if (keep) {
          ...
          std::string new_id = u.id + std::to_string(j + 1);
          std::vector<T> new_coords = u.coordinates;
          new_coords[j] = z_bar.coordinates[j];

          // Step 8: z^j(u_j) ← z̄
          // Steps 9-10: For k ≠ j, z^k(u_j) ← z^k(u) (inherited)
          std::vector<Point<T>> new_points = u.defining_points;
          new_points[j] = z_bar;

          P.emplace_back(std::move(new_id), std::move(new_coords),
                         std::move(new_points));
        }
      }
    }
  ...
  /**
   * @brief Algorithm 5: Update procedure based on redundancy avoidance (general
   * case).
   *
   * This algorithm avoids redundancy checks by keeping track of the sets Z^j(u)
   * of points that define each component of each local bound.
      ...
      for (std::size_t j = 0; j < dimensions_; ++j) {
        if (z_bar.coordinates[j] == u.coordinates[j]) {
          ...
          for (std::size_t k = 0; k < dimensions_; ++k) {
            if (k == j)
              continue;
            if (!detail::is_better<T, Sense>(z_bar.coordinates[k],
                                             u.coordinates[k])) {
              all_strict = false;
              break;
            }
    ...
    for (std::size_t idx : A_idx) {
      ...
      for (std::size_t j = 0; j < dimensions_; ++j) {
        ...
        if constexpr (Sense == Objective::MINIMIZE) {
          ...
          for (std::size_t k = 0; k < dimensions_; ++k) {
            if (k == j || u.defining_point_sets[k].empty())
              continue;
            T min_val = std::numeric_limits<T>::max();
            for (const auto &z : u.defining_point_sets[k]) {
              if (z.coordinates[j] < min_val)
                min_val = z.coordinates[j];
            }
            if (min_val > max_of_mins)
              max_of_mins = min_val;
          }
        } else {
          ...
          for (std::size_t k = 0; k < dimensions_; ++k) {
            ...
            for (const auto &z : u.defining_point_sets[k]) {
              if (z.coordinates[j] > max_val)
                max_val = z.coordinates[j];
            }
            if (max_val < max_of_mins)
              max_of_mins = max_val;
          }
        }
        ...
        if (keep) {
          ...
          // Step 10: Z^j(u_j) ← {z̄}
          std::vector<std::vector<Point<T>>> new_point_sets(dimensions_);
          new_point_sets[j] = {z_bar};
          ...
          P.emplace_back(std::move(new_id), std::move(new_coords),
                         std::vector<Point<T>>{}, std::move(new_point_sets));
        }
      }
    }
  ...
  void _update_naive(std::vector<LocalBound<T>> &current_bounds,
    ...
    // A bound is redundant when its search zone is a subset of another bound's
    // zone.
    //
    // - Minimization (upper bounds): the search zone of u is {z : z[i] < u[i]
    // ∀i}.
    //   If u_a ≤ u_b component-wise, then zone(u_a) ⊆ zone(u_b), so u_a is
    //   redundant. We keep the larger bound (u_b) and remove the smaller one
    //   (u_a).
    //
    // - Maximization (lower bounds): the search zone of u is {z : z[i] > u[i]
    // ∀i}.
    //   If u_a ≥ u_b component-wise, then zone(u_a) ⊆ zone(u_b), so u_a is
    //   redundant. We keep the smaller bound (u_b) and remove the larger one
    //   (u_a).
    //
    ...
    for (std::size_t i = 0; i < new_bounds.size(); ++i) {
      for (std::size_t j = i + 1; j < new_bounds.size(); ++j) {
        if (weakly_dominates<T, Sense>(new_bounds[i].coordinates,
                                       new_bounds[j].coordinates)) {
          redundant_idx.push_back(i);
        } else if (weakly_dominates<T, Sense>(new_bounds[j].coordinates,
                                              new_bounds[i].coordinates)) {
    ...
    redundant_idx.erase(std::unique(redundant_idx.begin(), redundant_idx.end()),
                        redundant_idx.end());
